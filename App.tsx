...
import { AppStep, ViralContent, GeneratedImage, StyleConfig, SignatureConfig } from './types';
...
const App: React.FC = () => {
  ...
  const handleGeneratePlan = async (inputTopic: string, images: string[]) => {
    ...
    try {
      ...
      alert(error.message || "Đã xảy ra lỗi khi tạo ý tưởng. Vui lòng kiểm tra API Key.");
      console.error(error);
  ...
  const handleGenerateImages = async (signature: SignatureConfig) => {
    ...
    try {
      ...
      for (const slide of viralContent.slides) {
        const url = await generateSlideImage(slide, signature, styleConfig);
        images.push({
          id: slide.id,
          url,
          slide
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-20 flex items-center justify-between">
          ...
          <div className="flex items-center space-x-4 sm:space-x-6">
             ...
             <button 
                ...
                className="p-2 rounded-full text-slate-500 hover:bg-slate-100 hover:text-royal-900 transition-colors"
